...
ArduinoQueue<int> temp(4);            // temporary queue for congested lanes
trafficLight lane[4];                 // traffic light object

const int timeYellowToRed = 2000;
...
void readFrontSensor() {
  lane[0].frontValue = analogRead(A0);
  lane[1].frontValue = analogRead(A1);
  lane[2].frontValue = analogRead(A2);
  lane[3].frontValue = analogRead(A3);
...
void readBackSensor() {
  lane[0].backValue = analogRead(A4);
  lane[1].backValue = analogRead(A5);
  lane[2].backValue = analogRead(A6);
  lane[3].backValue = analogRead(A7);
...
void setup() {
  Serial.begin(9600);
  // set lanes
  setLane(0, A0, A4, 2, 3, 4, HIGH, LOW, LOW, false, false, 0);
  setLane(1, A1, A7, 5, 6, 7, HIGH, LOW, LOW, false, false, 0);
  setLane(2, A2, A6, 8, 9, 10, HIGH, LOW, LOW, false, false, 0);
  setLane(3, A3, A5, 11, 12, 13, HIGH, LOW, LOW, false, false, 0);
  ...
  for (int i=0; i<4; i++) {
    if (lane[i].isBackTrig && check5seconds(lane[i].backSensorStart)) {
      ...
      for (int j = 0; j < temp.itemCount(); j++) {
        if (temp.getHead() != i+1) prioritySensor.enqueue(temp.dequeue());
        else {temp.dequeue();j++;}
      }
      lane[i].isBackTrig = false;
    }
  }
}
